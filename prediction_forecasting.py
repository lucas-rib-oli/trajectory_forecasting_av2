...
class TransformerPrediction ():
    def __init__(self, cfg, num_scenarios: int = 100) -> None:
        ...
        self.val_dataloader = DataLoader (self.val_data, batch_size=1, num_workers=self.num_workers, shuffle=True)
        # ---------------------------------------------------------------------------------------------------- #
    ...
    def predict(self):
        ...
        for idx, data in enumerate(self.val_dataloader):
            with torch.no_grad():                
                historic_traj: torch.Tensor = data['historic'].squeeze(1)
                future_traj: torch.Tensor = data['future'].squeeze(1)
                ...
                src_mask = torch.zeros((src_seq_len, src_seq_len),device=self.device).type(torch.bool)
                # ----------------------------------------------------------------------- #
                pred, conf = self.model (historic_traj, future_traj, src_mask=None, tgt_mask=None, src_padding_mask=None, tgt_padding_mask=None)
