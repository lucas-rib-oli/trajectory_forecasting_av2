...
parser.add_argument('--path_2_save', type=str, default='/home/lribeiro/TFM/resultados/embeddings', help='Path to the save the fig')
...
def get_model ():
    ...
    # ---------------------------------------------------------------------------------------------------- #
    # Get the model
    model = TransTraj (pose_dim=pose_dim, n_future=future_size, d_model=d_model, nhead=nhead, 
                            N=num_encoder_layers, dim_feedforward=dim_feedforward, dropout=dropout).to(device)
    name = 'best'
...
def _plot_actor_predictions (ax: plt.Axes, scenario, model: TransTraj):
    ...
    for track in scenario.tracks:
        ...
        with torch.no_grad():
            ...
            pred = model (historic_traj, future_traj, src_mask=None, tgt_mask=None)
            pred = pred.squeeze(0)
            pred = pred.cpu().numpy()
