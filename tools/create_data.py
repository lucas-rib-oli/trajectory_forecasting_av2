...
def normalice_heading (angle):
    ...
    return norm_angle
# ===================================================================================== #
def process_scenario (scenario_path, all_scene_data):
...
def prepare_data_av2(split: str):
    ...
    all_scene_data = manager.list()
    # ----------------------------------------------------------------------- #
    print (Fore.CYAN + 'Status: ' + Fore.WHITE + 'processing data ...')
    with multiprocessing.Pool() as pool:
        pool.starmap(process_scenario, [(path, all_scene_data) for path in all_scenario_files])
    all_scene_data = list(all_scene_data)
    # ----------------------------------------------------------------------- #
    # Print info
